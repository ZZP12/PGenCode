...
def bond_to_label(bond):
    # ZZ: returns 'AtomNum+MapNum+BondSmarts+AtomNum+MapNum'
    # ??? two same bonds all without map numbers in a reaction
...
def get_tagged_atoms_from_mols(mols):
    # ZZ: purpose of this function??? mixed atoms together???
    # ZZ: atoms and atomic map numbers
...
def get_tagged_atoms_from_mol(mol):
    # ZZ: tag is corresponding atomic map number
...
def get_changed_atoms(reactants, products):
    ...
    # ZZ: shouldn't atom tag (atomic map number) be unique? or '0' is the promiscuous tag?
    ...
    for j, reac_tag in enumerate(reac_atom_tags):
        if reac_tag not in changed_atom_tags:
            # ZZ: redundant? changed_atom_tags is a subset of prod_atom_tags
    ...
    [set_isotope_to_equal_mapnum(reactant) for reactant in reactants] # ZZ: why this?
...
def check_tetrahedral_centers_equivalent(atom1, atom2):
    ...
    # no idea what rdkit is doing, thus couldn't understand this code here
    ...
    for (add_if_match, template) in group_templates:
        ...
        for match in matches:
            ...
            for pattern_idx, atom_idx in enumerate(match):
                if pattern_idx in add_if_match:  # ZZ: what?
...
def process_an_example(reaction_smiles, super_general=False, v=False):
    ...
    # ZZ: (guess) input 'v' stands for 'verbose'
    ...
    try:
        ...
        if are_unmapped_product_atoms: # add fragment to template
            ...
            # total_partialmapped += 1 # ZZ: for sure a mistake here!!!
            for product in products:
                # Get unmapped atoms
                unmapped_ids = [
                    a.GetIdx() for a in product.GetAtoms() if not a.HasProp('molAtomMapNumber')
                ]
                if len(unmapped_ids) > 100: # MAXIMUM_NUMBER_UNMAPPED_PRODUCT_ATOMS: # ZZ: one more mistake
                    # Skip this example - too many unmapped!
                    print('warning: too many unmapped!') # added by ZZ
                    return
                ...
                if unmapped_ids:
                    extra_reactant_fragment += \
                        ...
                        # ZZ: inputs to 'atomSymbols' & 'bondSymbols' are correct? they include
                        # all atoms & bounds? (actually, no idea how to pick out only those
                        # bounds involved in the fragments) -> RDKit handles the harder questions!
            if extra_reactant_fragment:
                extra_reactant_fragment = extra_reactant_fragment[:-1]
                # ZZ: get rid of the last '.', so the whole string is seperated by '.'
            ...
            # ZZ: remove repeated extra fragments
        ...
        if None in reactants + products: # ZZ: already checked inside the 1st try block
...
if __name__ == '__main__':
    ...
    # r1 = Chem.MolFromMolFile('C00025.txt')
    # p1 = Chem.MolFromMolFile('C00624.txt')
    # sr1 = Chem.MolToSmiles(r1)
    # sp1 = Chem.MolToSmiles(p1)
    # srxn = sr1 + '>>' + sp1
    # print(srxn)
    # process_an_example(srxn)
    # print('======example 2======')
    # process_an_example('[C:1][#6:2]([H])=[O:3]>>[C:1][#6:2](-[#16][H])=[O:3]')
    # print('======example 3==================')
    # rxn = process_an_example('[#6:5][#6:3]([#6:6])([#6:7])[#6:1]([H:11])([H:10])'
    #                    '[#6:2]([H:9])([#6:8])[#1:4]>>[H:11][#6:1]([H:10])'
    #                    '=[#6:2]([H:9])[#6:8].[#6:5][#6:3]([#6:6])([#6:7])[#1:4]',
    #                    v=True)
    # print(rxn)
    # print()
    # rxn = process_an_example('[#6:5][#6:3]([#6:6])([#6:7])[#6:1]([H:11])([H:10])'
    ...
    from rdkit import Chem
    # a more complex testing case
    print('====a more complex test=====')
    # prodSmart = '[#6:2](-[#6:4]-[#6:5]-[#6:6]=[#8:7])(-[#8:1])=[#8:3]'
    # reacSmart = '[#6]-[#6](-[#6])(-[#6]-[#8]-[P](=[#8])(-[#8])-[#8]-[P](=[#8])(-[#8])-[#8]-[#6]-[#6]-1-[#6](-[#6](-[#6](-[#8]-1)-n2cnc3c(ncnc23)-[#7])-[#8])-[#8]-[P](=[#8])(-[#8])-[#8])-[#6](-[#6](=[#8])-[#7]-[#6]-[#6]-[#6](=[#8])-[#7]-[#6]-[#6]-[#16]-[#6:6](-[#6:5]-[#6:4]-[#6:2](-[#8:1])=[#8:3])=[#8:7])-[#8]'
    # rxnStr = reacSmart + '>>' + prodSmart
    # rxn = process_an_example(rxnStr, v=True)
    # print('output 1 \n {}'.format(rxn))
    # print('-------------------------')
    # rxn = process_an_example(rxnStr, super_general=True, v=True)
    # print('output 2 \n {}'.format(rxn))
    # print('-------------------------')
    # rxn = process_an_example(prodSmart + '>>' + reacSmart, super_general=True, v=True)
    # print('output 3 \n {}'.format(rxn))

    # ? a failure below
    # prodMol = Chem.MolFromSmarts(prodSmart)
    # [at.UpdatePropertyCache() for at in prodMol.GetAtoms()]
    # prodSmart = Chem.MolToSmarts(Chem.AddHs(prodMol))
    # print(prodSmart)
    # reacMol = Chem.MolFromSmarts(reacSmart)
    # [at.UpdatePropertyCache() for at in reacMol.GetAtoms()]
    # reacSmart = Chem.MolToSmarts(Chem.AddHs(reacMol))
    # print(reacSmart)
    # rxnStr = reacSmart + '>>' + prodSmart
    # print('-----------------')
    # print(rxnStr)
    ...
    # CoA reaction AAM from RDT
    # rxnStr = '[O:1]=[C:2]([OH:3])[CH2:4][CH2:5][C:6](=[O:7])[S:8][CH2:9][CH2:10][NH:11][C:12](=[O:13])[CH2:14][CH2:15][NH:16][C:17](=[O:18])[CH:19]([OH:20])[C:21]([CH3:22])([CH3:23])[CH2:24][O:25][P:26](=[O:27])([OH:28])[O:29][P:30](=[O:31])([OH:32])[O:33][CH2:34][CH:35]1[O:36][CH:37]([n:38]2[cH:39][n:40][c:41]3[c:42]([n:43][cH:44][n:45][c:46]23)[NH2:47])[CH:48]([OH:49])[CH:50]1[O:51][P:52](=[O:53])([OH:54])[OH:55]>>[OH:3][C:2](=[O:1])[CH2:4][CH2:5][CH:6]=[O:7]'
    # rxn = process_an_example(rxnStr, super_general=True, v=True)
    # print('output 4 \n {}'.format(rxn))

    # an atom-balanced example AAM from RDT
    rxnStr = '[O:1]=[C:2]([O:3][CH2:4][CH3:5])[CH:6]1[C:7](=[O:8])[CH2:9][CH2:10][CH2:11]1.[O:12]=[C:13]([CH:14]=[CH2:15])[CH3:16]>>[O:1]=[C:2]([O:3][CH2:4][CH3:5])[C:6]1([C:7](=[O:8])[CH2:9][CH2:10][CH2:11]1)[CH2:15][CH2:14][C:13](=[O:12])[CH3:16]'
    rxn = process_an_example(rxnStr, super_general=True, v=True)
    print('output 4 \n {}'.format(rxn))

    # no an easy job to find a testing example LOL
    # is this going to give me what I want? rules with precursors fully described!
    # what such a template looks like? no AAM in products
    # reaction centers: in atom-balanced reaction, are atoms that change connections
    #                   in unbalanced reaction (our case), change connections + no aam + ...
